...
def run_test(prob_path: str = None, problem_list: List[str] = None, prob_index: int = None,
    ...
    if os.path.exists(os.path.join(root, "input_output.json")):
        with open(os.path.join(root, "input_output.json")) as f:
            in_outs = json.load(f)  # 获取包含输入和输出的一个字典
            if debug:
                print(f"test cases json = {in_outs['inputs']} {in_outs['outputs']}")

            if in_outs.get("fn_name") is None:  # 不包含函数，以标准输入来测试
        ...
        for index, inputs in enumerate(in_outs["inputs"]):
            ...
            try:
                if isinstance(in_outs["outputs"][index], dict):
                    in_outs["outputs"][index] = [{int(k): v for k, v in in_outs["outputs"][index].items()}]  # outputs=
            except:
                True
            try:
            ...
            if which_type == CODE_TYPE.call_based:  # Call-based
                ...
                # 运行时错误返回[-1]
                    ...
                    print(f"Standard input runtime error or time limit exceeded error = {e}")  # todo:英文描述是不是反了
                    results.append(-1)
                    continue
                faulthandler.disable()
                signal.alarm(0)
