...
from codegen.modeling_codegen import CodeGenForCausalLM
...
########################################################################
# sample
def sample(device, model: CodeGenForCausalLM, tokenizer, context, pad_token_id,
           num_return_sequences=1, temp=0.2, top_p=0.95, max_length_sample=128, max_length=2048):
    input_ids = tokenizer(context, truncation=True, padding=True, max_length=max_length, return_tensors='pt',).input_ids
...
def load(args):
    ...
    if device.type == "cpu":
        args.no_fp16 = False
        # args.no_fp16 = True  # "LayerNormKernelImpl" not implemented for 'Half'
...
# (4) sample
def gen_code(raw_text, model, tokenizer, args):
    with print_time('sampling'):
        # 得到文本列表(num_return_sequences:sample数量; temp:温度; max_length_sample:sample长度)
        completion = sample(device=args.device, model=model, tokenizer=tokenizer, context=raw_text,
                            pad_token_id=args.pad, num_return_sequences=args.batch_size, temp=args.temp,
                            top_p=args.p, max_length_sample=args.max_length)
        # 对每个句子进行处理
        for idx in range(len(completion)):
            split_res = completion[idx].split("ANSWER:\n")
            if len(split_res) > 1:
                completion[idx] = split_res[1]
            completion[idx] = truncate(completion[idx])

        # print('=' * 100)
        # print(completion[0])
        # print('=' * 100)

        return completion  # 返回结果列表,每个元素都是代码
